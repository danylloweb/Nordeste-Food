...
class Pedidos extends CI_Controller {
    ...
    public function __construct() {
        ...
        $this->template->set('body_class', 'pg-interna pg-pedido');
    ...
    public function add_marmita() {
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    	    ...
    	    if (count(array_filter($dados_bd)) <= 3) {
                $this->session->set_flashdata('erro', 'Insira pelo menos três itens a marmita!');
                redirect('pedidos');
            } else {
                ...
                if ($this->refeicao->inserir($dados_bd)) {
                    ...
                    $this->session->set_flashdata('sucesso', 'Sua marmita foi adicionada ao carrinho com sucesso! Você pode adicionar outra marmita ou ir para Próxima Etapa.');
                    redirect('pedidos');

                } else {
                    $this->session->set_flashdata('erro', 'Não foi possível adicionar a marmita, entre em contato conosco.');
                    redirect('pedidos');
                }
            }
        }
